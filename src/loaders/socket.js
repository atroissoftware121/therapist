...
async function handleSocket() {
  io.on('connection', async (socket) => {
    ...
    socket.on('therapist-active', async (data) => {
      await updateQuery(
        therapistModel,
        { _id: data.therapistId },
        { isOnline: true }
      );
      io.emit(
        'list-of-active-therapist',
        await findQuery(therapistModel, { isOnline: true })
      );
    });
    ...
    socket.on('event-emit-call-connected', (individualId) => {
      io.to(individualId).emit(
        'accept-call-emit',
        'we will connect after some time'
      );
    });
    ...
    socket.on('individual-show-to-therapist', async (therapistId) => {
      io.emit(
        'list-of-individual-for-call',
        await findQuery(chatDetailsModel, {
...
const listOfMessages = async (userId, socket) => {
  const [isChatExisted] = await findQuery(chatDetailsModel, {
    receiverId: userId,
    chatType: 'message',
  });
  if (!isChatExisted) {
    socket.emit('chat-not-found', {
      message: 'Chat does not exist with this userId',
    });
  } else {
...
const updatedList = async (data, io) => {
  ...
  io.to(therapistsId).emit('refresh-chat-data', {
    data: [updateChatDetails.individualDetails],
  });
};
...
const updatedCallList = async (data, io) => {
  ...
  io.to(therapistsId).emit('refresh-call-data', {
    data: [updateChatDetails.individualDetails],
  });
};
...
const refreshCallListsEvent = async (data, therapistsId) => {
  io.to(therapistsId).emit('refresh-call-lists', {
    data: data.individualDetails,
  });
};
...
const chatDetailsEvent = async (data, messageData, individualData) => {
  if (data.chatType === 'message') {
    io.to(data.receiverId).emit('chat-details', {
      data: [messageData?.individualDetails],
      image: individualData.image,
    });
  } else {
    io.to(data.receiverId).emit('chat-details-for-call', {
      data: [messageData?.individualDetails],
      image: individualData.image,
      timing: data.timing,
    });
